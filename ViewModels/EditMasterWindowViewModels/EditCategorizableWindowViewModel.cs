...
namespace YukaLister.ViewModels.EditMasterWindowViewModels
{
	internal class EditCategorizableWindowViewModel<T> : EditMasterWindowViewModel<T> where T : class, IRcCategorizable, new()
	{
		...
		public override void Initialize()
		{
			...
			{
				YlModel.Instance.EnvModel.LogWriter.ShowLogMessage(TraceEventType.Error, "カテゴリー持ち詳細編集ウィンドウ初期化時エラー：\n" + excep.Message);
		...
		private void ContextMenuButtonSelectCategoryItem_Click(Object sender, RoutedEventArgs routedEventArgs)
		{
			...
			{
				YlModel.Instance.EnvModel.LogWriter.ShowLogMessage(TraceEventType.Error, "カテゴリー選択メニュークリック時エラー：\n" + excep.Message);
				YlModel.Instance.EnvModel.LogWriter.ShowLogMessage(Common.TRACE_EVENT_TYPE_STATUS, "　スタックトレース：\n" + excep.StackTrace);
			}
		}
	}
