...
internal abstract class YlContext : DbContext
{
	...
	{
		Log.Information(_databaseName + "データベース初期化中...");
		...
		if (Properties == null)
		{
			Log.Information(_databaseName + "データベースを初期化できませんでした。");
			return;
		}
		DbCommon.UpdateProperty(this, Properties);

		Log.Information(_databaseName + "データベースを初期化しました。");
	}
	...
	public void SetJournalModeIfNeeded()
	{
		...
		{
			Log.Error(_databaseName + "データベースの接続を取得できませんでした。");
			return;
		}
		...
		Log.Information(_databaseName + "データベースのジャーナルモードを DELETE にしました。");
	}
